...
class IPAdapterPlusXL(IPAdapter):
    ...
    def get_image_embeds(self, clr_ref_img=None, clr_texture_ref_img=None, texture_ref_img=None,
                         ...
                         subscale=0., color_scale=1, texture_scale=0,
                         texture2_scale=0.0, geometry_sub_scale=0.0, texture_geometry_decouple=None, geometry_scale=0.0,
                         geometry_color_decouple=0.0, texture_color_decouple=0.0,
                         ...
                         punish_weight=0.0, punish_type=None):
        ...
        # No-orthogonal ablation: keep only the reference preprocessing done by
        # the runner, then feed raw CLIP hidden states into the three streams.
        clr_clip_image_embeds = c_ori
        txr_clip_image_embeds2 = t_ori
        geo_clip_image_embeds = g_ori

        color_stream_embeds = None

        if clr_clip_image_embeds is not None:
            color_stream_embeds = clr_clip_image_embeds * color_scale
            if clr_txr_clip_embeds is not None and subscale != 0:
