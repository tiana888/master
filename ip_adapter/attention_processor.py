...
#     return phase_mix
def get_base_phase_mix_table(save_name):
    # 1. Down Blocks: 絕對幾何鎖定區
    if "down_blocks" in save_name:
        return {
            "front": (0.00, 1.00, 0.00), # 100% Geometry
            "mid":   (0.10, 0.80, 0.10),
            "back":  (0.00, 0.50, 0.50), # 後期轉入少量 Texture 輔助邊緣
        }
    
    # 2. Mid Block: 全局色彩與氛圍橋樑
    if "mid_block" in save_name:
        return {
            "front": (0.10, 0.80, 0.10), # 強化前期幾何對齊
            "mid":   (0.30, 0.50, 0.20), # 色彩開始接管
            "back":  (0.50, 0.00, 0.5), # 後期徹底移除幾何，確保氛圍穩定
        }
    
    # 3. Up Blocks 0: 黃金渲染區 (外觀與解耦的核心)
    if "up_blocks.0" in save_name:
        return {
            "front": (0.35, 0.40, 0.25), # 前期仍需幾何穩定位置
            "mid":   (0.40, 0.20, 0.40), # 幾何開始讓位給色彩 (解耦關鍵)
            "back":  (0.50, 0.00, 0.50), # 後期完全關閉幾何，消除雜訊
        }
    
    # 4. 其他 Up Blocks: 細節填充與平滑
    if "up_blocks" in save_name:
        return {
            "front": (0.30, 0.40, 0.30),
            "mid":   (0.40, 0.30, 0.30),
            "back":  (0.40, 0.00, 0.60), # 強化 Texture 權重，確保收斂細緻
        }
        
    return {
        "front": (1.0, 0.0, 0.0), "mid": (1.0, 0.0, 0.0), "back": (1.0, 0.0, 0.0),
    }

def get_phase_mix_table(save_name):
    phase_mix = dict(get_base_phase_mix_table(save_name))
    # 強力執行 Geometry-Free 策略：針對黃金渲染層
    if "up_blocks.0.attentions.1" in save_name and save_name.endswith("attn2.processor"):
        # 在中期就開始大幅削弱幾何，後期徹底歸零
        phase_mix["front"] = (0.50, 0.00, 0.50)
        phase_mix["mid"] = (0.50, 0.00, 0.50)
        phase_mix["back"] = (0.50, 0.00, 0.50)
    # if "up_blocks.0.attentions.2" in save_name and save_name.endswith("attn2.processor"):
    #     # 這裡不需要在中期就歸零幾何，但「後期」必須歸零以防網格雜訊
    #     # 同時大幅拉高 Texture 的比例
    #     phase_mix["mid"] = (0.10, 0.00, 0.90)  # 保留少量幾何維持邊界，增加紋理
    #     phase_mix["back"] = (0.10, 0.00, 0.90) # 後期徹底移除幾何，全力衝刺紋理 (0.80)
    return phase_mix
